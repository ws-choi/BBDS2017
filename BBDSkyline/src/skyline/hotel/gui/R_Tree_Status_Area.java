...
public class R_Tree_Status_Area extends JPanel implements CWS_producer {
	...
	public JTextField num_of_page_field;
	public R_Tree_Status_Area(RTree tree) {
		...
		gbc_btnDestroyCreate.insets = new Insets(0, 0, 5, 0);
		...
		gbc_lblHeightOfTree.insets = new Insets(0, 0, 5, 5);
		...
		JLabel lblOfPage = new JLabel("# of Page Access");
		lblOfPage.setFont(new Font("����", Font.BOLD, 12));
		GridBagConstraints gbc_lblOfPage = new GridBagConstraints();
		gbc_lblOfPage.weighty = 1.0;
		gbc_lblOfPage.anchor = GridBagConstraints.EAST;
		gbc_lblOfPage.insets = new Insets(0, 0, 0, 5);
		gbc_lblOfPage.gridx = 0;
		gbc_lblOfPage.gridy = 4;
		add(lblOfPage, gbc_lblOfPage);
		
		num_of_page_field = new JTextField();
		GridBagConstraints for_page_access = new GridBagConstraints();
		for_page_access.fill = GridBagConstraints.HORIZONTAL;
		for_page_access.gridx = 1;
		for_page_access.gridy = 4;
		add(num_of_page_field, for_page_access);
		num_of_page_field.setColumns(10);
		checker.start();
	...
	private void init() {
		...
		gridBagLayout.rowWeights = new double[]{0.0, Double.MIN_VALUE, 0.0, 0.0, 0.0};
...
class infinit_checker implements Runnable {
	...
	public void run() {
		...
		while(true){
			...
			rsa.num_of_page_field.setText(rsa.rtree.page_access+"");
			try {
				Thread.sleep(100);
