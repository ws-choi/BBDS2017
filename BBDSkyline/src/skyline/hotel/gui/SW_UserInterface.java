...
import skyline.hotel.experiment.Normal_Skyline;
...
import skyline.hotel.hotel.HtS_Event;

import data.Point_Float;
import data.generator.DataGenerator;

public class SW_UserInterface extends JFrame implements CWS_producer, CWS_consumer
{
	...
	public SW_UserInterface() {
		...
		SKY_Constants.connect(hotel_area,this, SKY_Constants.hts);
		SKY_Constants.connect(input_controller, hotel_area, SKY_Constants.ith);
		SKY_Constants.connect(dataGen_gui, this, SKY_Constants.dts);
		SKY_Constants.connect(this, dataGen_gui, SKY_Constants.std);
		SKY_Constants.connect(r_tree_status_area, this, SKY_Constants.rts);
		SKY_Constants.connect(r_tree_status_area, hotel_area, SKY_Constants.rth);
	}
	...
	public void consume_something(int port, My_Event my_Event) {
		...
		case SKY_Constants.hts:
			...
			else if ( my_Event instanceof Skyline_Query_Event)
			{
				...
				(new Thread( new Normal_Skyline(tree, hotel_area,(Skyline_Query_Event) my_Event, 1000) ) ).start();
